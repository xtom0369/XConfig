...
3. **添加hello_world.byte配置表**。将Assets\Example\Config\csv_template.bytes文件拷贝，重命名为hello_world.bytes

4. **导出配置表代码**。执行Unity菜单【**XConfig/Generate Code**】，会在Assets\Example\Generate\Code文件夹中生成**HelloWorldTable**（表类）和**HelloWorldRow**（行类）。修改表头（比如字段名/字段类型）后需要重新导出代码

5. **导出为二进制文件**。执行Unity菜单【**XConfig/Generate Binary**】，会早Assets\Example\Generate\Bin中生成相应的**hello_world.bytes**二进制文件。每次修改配置表数据后都需要重新导出。
