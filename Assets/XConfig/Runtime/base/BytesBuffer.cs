...
namespace XConfig 
{
    ...
    {
        public const int PRECISION = 1000;//命令如果携带浮点数，则先用这个精度放大，经过网络传输，使用时再按这个精度缩小
        ...
        public byte[] ReadBytes(int count)
        {
            ...
            {
                ret[i] = _buffer[_readPos + i];
            }
            _readPos += sizeof(byte) * count;
            return ret;
        }
        ...
        public void WriteFloat(float value)
        {
            ...
            DebugUtil.Assert(temp.Length == sizeof(float), "{0} != {1}", temp.Length, sizeof(float));
        ...
        public void WriteFixQuaternion(Quaternion value)
        {
            WriteFloat(value.x);
            WriteFloat(value.y);
            WriteFloat(value.z);
        ...
        public Quaternion ReadFixQuaternion()
        {
            var x = ReadFloat();
            var y = ReadFloat();
            var z = ReadFloat();
        ...
        public void WriteString(string str)
        {
            ...
            else
            {
                ...
                DebugUtil.Assert(strBytes.Length < ushort.MaxValue, "字符串过长：{0}:{1}", strBytes.Length, str);
        ...
        void Expand(int sz)
        {
            if (_capacity - _size < sz)
            {
                ...
                {
                    DebugUtil.LogError("object is too large (>" + encode_max_size + ")");
                }
