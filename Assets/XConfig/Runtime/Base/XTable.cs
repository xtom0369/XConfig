...
namespace XConfig
{
    ...
    public abstract class XTable<TKey, TRow> : XTable where TRow : XRow<TKey>, new()
    {
        ...
        List<TRow> _rows;
        Dictionary<TKey, TRow> mainKey2Row;
        ...
        public override void ReadFromBytes(BytesBuffer buffer)
        {
            ...
            {
                TRow row;
                if (i >= oldCount) { row = new TRow(); row.fileName = name; _rows.Add(row); }
                else row = _rows[i];
        ...
        public TRow GetRow(TKey mainKey)
        {
        ...
        public bool TryGetRow(TKey mainKey, out TRow row) 
        { 
    ...
    public abstract class XTable<TKey1, TKey2, TRow> : XTable where TRow : XRow<TKey1, TKey2>, new()
    {
        ...
        List<TRow> _rows;
        Dictionary<TKey1, List<TRow>> _firstKey2Rows;
        Dictionary<TKey1, Dictionary<TKey2, TRow>> _mainKey2Row;
        ...
        public List<TRow> GetRows(TKey1 key1)
        {
        ...
        public TRow GetRow(TKey1 key1, TKey2 key2)
        {
        ...
        public bool TryGetRow(TKey1 key1, TKey2 key2, out TRow row) 
        {
            row = null;
        ...
        public bool TryGetRows(TKey1 key1, out List<TRow> rows)
        {
            rows = null;
            if (!_mainKey2Row.TryGetValue(key1, out var secondKey2Row))
                return false;
