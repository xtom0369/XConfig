...
public class CsvClassFileExporter : TextFileExporter
{
 ...
 */";
    ...
    public CsvClassFileExporter(string outFilePath, TextFileImporter importer, CsvFileContext context) : base(outFilePath, importer)
    {
        this.context = context;
    }
    public CsvFileImporter csvImporter { get { return importer as CsvFileImporter; } }
    ...
    void WriteAllTableInitCompleteFunction()
    {
        ...
        {
            CsvFileImporter rootImporter = csvImporter.parentFileImporter;
    ...
    void WriteRowClass()
    {
        ...
        List<CsvFileImporter> parentImporters = new List<CsvFileImporter>();
        CsvFileImporter parentImporter = csvImporter.parentFileImporter;
        ...
        foreach (CsvFileImporter importer in parentImporters)
        {
        ...
        for (int i = 0; i < csvImporter.keys.Length; i++)
        {
            ...
            if (CsvFileImporter.IsFilterNotUseColoum(flag)) continue;
        ...
        for (int i = 0; i < csvImporter.keys.Length; i++)
        {
            ...
            if (string.IsNullOrEmpty(key)) continue;
            string flag = csvImporter.flags[i];
            bool isNeedGenerateField = CsvFileImporter.IsFilterNotUseColoum(flag);
            string type = csvImporter.types[i];
            //string defaultValue = csvImporter.defaults[i];
            if (isInheritClass && flag.Contains("M"))
            {
    ...
    void WriteRowFromBytesFunction()
    {
        ...
        for (int i = 0; i < csvImporter.keys.Length; i++)
        {
            ...
            bool isEditorFields = CsvFileImporter.IsFilterNotUseColoum(flag);
    ...
    void WriteReference(string key, string type, string flag, bool isEditorMode = false)
    {
        ...
        CsvFileImporter importer = context.fileName2ImporterDic[type] as CsvFileImporter;
        string lowerName = ConvertUtil.ToFirstCharlower(key);
    ...
    void WriteListReference(string key, string type, string flag, bool isEditorMode = false)
    {
        ...
        CsvFileImporter importer = context.fileName2ImporterDic[referenceTableName] as CsvFileImporter;
        if (importer.majorKeyType == EnumTableMojorkeyType.INT)
