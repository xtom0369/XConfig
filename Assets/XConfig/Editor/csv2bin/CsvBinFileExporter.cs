...
public class CsvBinFileExporter : BinFileExporter
{
    const int PRECISION = 10000;//配置表的小数精度

    public CsvFileImporter csvImporter { get { return importer as CsvFileImporter; } }
    CsvFileContext context;
    int lineNumber;
    public CsvBinFileExporter(string outFilePath, TextFileImporter importer, CsvFileContext context, BytesBuffer buffer) : base(outFilePath, importer, buffer)
    {
    ...
    void WriteRow(string[] keys, string[] types, string[] values, string[] flags, bool isParentRow)
    {
        ...
        for (int i = 0; i < types.Length; i++)
        {
            ...
            if (!string.IsNullOrEmpty(key))//有效列
            {
                ...
                string type = types[i];
                string flag = flags[i];
                bool isNeedGen = CsvFileImporter.IsFilterNotUseColoum(flag) == false;
#if UNITY_STANDALONE
                isNeedGen = true;
#endif
                //前后不能含有空白字符
